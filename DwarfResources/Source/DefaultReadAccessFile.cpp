...
df::DefaultReadAccessFile::~DefaultReadAccessFile() {
	fclose(m_File);
}

auto df::DefaultReadAccessFile::MapData(size_t offset /*= 0*/, size_t size /*= 0*/) -> df::FileData {
	if (size == 0) {
		size = m_Size - offset;
	}
...
auto df::DefaultReadAccessFile::GetSize() const -> size_t {
	return m_Size;
}
