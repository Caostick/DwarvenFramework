...
namespace df {
	...
	struct ContainsList<T, TypeList<Args...>>
		: std::integral_constant<bool, df::Contains<typename T::Head, TypeList<Args...>>::value && ContainsList<typename T::Tail, TypeList<Args...>>::value> {};
	...
	template<typename T>
	struct FindType<T, EmptyTypeList>
		: Constants::npos {
	};

	template<typename ...Args>
	struct FindType<_private::Void, TypeList<Args...>>
		: Constants::npos {};

	template<typename T, typename ...Args>
	struct FindType<T, TypeList<Args...>>
